...
namespace VV.Utility
{
    public static class GameObjectExtensions
    {
        public static Vector3 WorldPointToRectTransformPosition(this RectTransform rt, Vector3 p, Camera cam = null)
        {
            ...
            Vector2 pos;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(rt, new Vector2(screenPoint.x, screenPoint.y), null,
                out pos);
        ...
        public static Bounds GetBoundsRecursive(this GameObject go)
        {
            ...
            if(go.TryGetComponent(out Renderer renderer)) b.Encapsulate(renderer.bounds);
            List<Renderer> rList = go.GetComponentsInChildren<Renderer>().ToList();
            foreach (var r in rList.Where(r => !r.gameObject.GetComponent<ParticleSystem>()))
